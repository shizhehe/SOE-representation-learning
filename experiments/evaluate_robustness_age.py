...
model_id = f"VN_Net_fold_{fold}_age"
#model_id = "baseline_rig_eval"
#model_id = "baseline_rig_eval_retrained"
#model_id = f"baseline_fold_{fold}"
model_name = 'VN_Net'
#model_name = 'CLS'
...
ckpt_path = os.path.join(ckpt_folder, dataset_name, model_name, f"week{week}", task, model_id)
...
def evaluate_rotation(phase='val', set='val', save_res=True, info='', model=model, cpkt_path=ckpt_path):
    ...
    with torch.no_grad():
        ...
        for key in pred_list_rot.keys():
            ...
            bacc = compute_classification_metrics(label_list_rot[key], pred_list_rot[key], dataset_name, subj_list_postfix, task, std_age, mean_age)

        print("--- Overall performance ---")
        label_list = np.concatenate(label_list, axis=0)
        pred_list = np.concatenate(pred_list, axis=0)
        bacc = compute_classification_metrics(label_list, pred_list, dataset_name, subj_list_postfix, task, std_age, mean_age)
