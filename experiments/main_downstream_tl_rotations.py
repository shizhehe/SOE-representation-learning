...
pretext_fold = 0
model_name = 'VN_Net'
model_ckpt = f"{base_name.format(fold=fold)}_milder_robustness_{'normalized' if normalize_matrix else 'unnormalized'}{'_frozen' if froze_encoder else ''}_robustness"
    ...
    for epoch in range(start_epoch + 1, start_epoch + epochs):
        ...
        for iter, sample in enumerate(trainDataLoader, 0):
            for _ in range(1):
                ...
                if global_iter % 1 == 0:
                    # pdb.set_trace()
                    print('Epoch[%3d], iter[%3d]: loss=[%.4f], cls=[%.4f], dis=[%.4f], rotation=[%.4f]' \
                            % (epoch, iter, loss.item(), loss_cls.item(), 0, rotation))
